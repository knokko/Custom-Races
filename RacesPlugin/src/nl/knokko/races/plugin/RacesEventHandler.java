...
public class RacesEventHandler implements Listener {
	...
	public static void startUpdater(){
		...
		for(String r : races){
			...
			if(race.needsUpdate()){
				...
				Bukkit.getScheduler().scheduleSyncRepeatingTask(RacesPlugin.instance(), updater, 0, race.getUpdatePeriod());
				updaters.put(race, updater);
			}
		}
		Bukkit.getScheduler().scheduleSyncRepeatingTask(RacesPlugin.instance(), new EffectUpdater(), 0, 40);//TODO something is wrong with the particles, but it seems to be a bug in Bukkit
	}
	...
	public void onItemUse(PlayerInteractEvent event){
		...
		if(action == Action.LEFT_CLICK_AIR || action == Action.LEFT_CLICK_BLOCK)
			race.raceLeftClick(new RaceLeftClickEvent(RaceManager.getRacePresentor(event.getPlayer(), race), itemFromBukkitMaterial(event.getMaterial())));
		if(action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK)
			race.raceRightClick(new RaceRightClickEvent(RaceManager.getRacePresentor(event.getPlayer(), race), itemFromBukkitMaterial(event.getMaterial())));
	}
	...
	public void onBlockBreak(BlockBreakEvent event){
		...
		RaceBreakBlockEvent raceEvent = new RaceBreakBlockEvent(RaceManager.getRacePresentor(event.getPlayer(), race), blockFromBukkitMaterial(b.getType()), b.getX(), b.getY(), b.getZ());
	...
	public void applyRaceEffects(EntityDamageByEntityEvent event){
		if(event.getDamager() instanceof Player){
			...
			*/
			...
			if(event.getEntity() instanceof Player){
				...
				RaceAttackRaceEvent playerEvent = new RaceAttackRaceEvent(RaceManager.getRacePresentor(damager, race, cond), RaceManager.getRacePresentor(victim), damage);
		...
		if(event.getEntity() instanceof Player){
			...
			if(event.getDamager() instanceof LivingEntity)
				applyEffects((LivingEntity) event.getDamager(), race.getOnHitPotionEffects(stats));
			checkEffects(target);
			*/
			double damage = event.getDamage();
			RaceHurtByEntityEvent entityEvent = new RaceHurtByEntityEvent(RaceManager.getRacePresentor(target, race, stats), RaceManager.getEntityPresentor(event.getDamager()), ReflectedCause.fromBukkitCause(event.getCause()), damage);
			...
			if(event.getDamager() instanceof Player){
				...
				RaceHurtByRaceEvent playerEvent = new RaceHurtByRaceEvent(RaceManager.getRacePresentor(target, race, stats), RaceManager.getRacePresentor(attacker), ReflectedCause.fromBukkitCause(event.getCause()), damage);
	...
	public void cancelItemEffects(PlayerItemConsumeEvent event){
		...
		if(type == Material.POTION || type == Material.MILK_BUCKET || type == Material.ROTTEN_FLESH || type == Material.SPIDER_EYE || type == Material.RAW_CHICKEN || type == Material.GOLDEN_APPLE)
	...
	public void onRaceDie(PlayerDeathEvent event){
		...
		if(damageEvent instanceof EntityDamageByEntityEvent){
			...
			race.raceKilledByEntity(new RaceKilledByEntityEvent(presentor, RaceManager.getEntityPresentor(damageByEntityEvent.getDamager()), cause));
	...
	public void onEntityDie(EntityDeathEvent event){
		...
		if(damageEvent instanceof EntityDamageByEntityEvent){
			...
			if(damageByEntityEvent.getDamager() instanceof Player){
				...
				race.raceKillEntity(new RaceKillEntityEvent(presentor, RaceManager.getEntityPresentor(event.getEntity())));
			}
		}
	}
	...
	private static void updatePlayerAttributes(Player player){
		...
		player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).addModifier(new AttributeModifier(ID_SPEED, "rpg.race.speed", race.getSpeedMultiplier(stats) - 1, Operation.ADD_SCALAR));
		...
		public void run() {
			...
			for(PotionEffect effect : newEffects){
				if(!effect.getType().isInstant() && !hadPotionEffect(effect.getType())){
					...
					if(resistance >= 1)
						...
						player.removePotionEffect(effect.getType());
						player.addPotionEffect(new PotionEffect(effect.getType(), (int) (effect.getDuration() * (1 - resistance)), effect.getAmplifier(), effect.isAmbient(), effect.hasParticles(), effect.getColor()));
					}
				}
			}
			...
			for(PermanentEffect permEffect : pEffects){
				if(!player.hasPotionEffect(PotionEffectType.getByName(permEffect.getType().getType())))
					player.addPotionEffect(new PotionEffect(PotionEffectType.getByName(permEffect.getType().getType().toUpperCase()), Integer.MAX_VALUE, permEffect.getAmplifier(), permEffect.isAmbient(), permEffect.hasParticles(), toBukkitColor(permEffect.getColor())));
			}
		}
	...
	private static class RaceUpdater implements Runnable {
	...
	public static PotionEffect toBukkitEffect(ReflectedEffect effect){
		return new PotionEffect(toBukkitEffectType(effect.getType()), effect.getDuration(), effect.getAmplifier(), effect.isAmbient(), effect.showParticles(), toBukkitColor(effect.getColor()));
	}

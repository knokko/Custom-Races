...
public class FunctionMultiDivide extends Function {
	...
	public FunctionMultiDivide(Function[] functionsToSum) {
		if(functionsToSum.length < MIN_AMOUNT)
			throw new IllegalArgumentException("At least two functions are required to divide: (" + functionsToSum.length + ")");
		if(functionsToSum.length > MAX_AMOUNT)
			throw new IllegalArgumentException("Too many functions to divide: " + functionsToSum.length);

...
public class FunctionMultiSubstract extends Function {
...
private static final byte AMOUNT_BITS = 4;
	...
	public FunctionMultiSubstract(Function[] functionsToSum) {
		if(functionsToSum.length < MIN_AMOUNT)
			throw new IllegalArgumentException("At least two functions are required to substract: (" + functionsToSum.length + ")");
		if(functionsToSum.length > MAX_AMOUNT)
			throw new IllegalArgumentException("Too many functions to substract: " + functionsToSum.length);
		functions = functionsToSum;
	}

	public FunctionMultiSubstract(BitBuffer buffer) {
		super(buffer);
